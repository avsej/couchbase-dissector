...
static void
dissect_couchbase_udp (tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
  gint        offset = 0;
  guint8      magic;
  ...
  if (match_strval (magic, magic_vals) != NULL) {
    dissect_couchbase (tvb, pinfo, tree);
  } else {
...
proto_register_couchbase (void)
{
  static hf_register_info hf[] = {
    ...
    { &hf_command,
      { "Command", "couchbase.command",
        FT_STRING, BASE_NONE , NULL, 0x0,
        NULL, HFILL } },

    { &hf_subcommand,
      { "Sub command", "couchbase.subcommand",
        FT_STRING, BASE_NONE, NULL, 0x0,
        "Sub command if any", HFILL } },

    { &hf_flags,
      { "Flags", "couchbase.flags",
        FT_UINT16, BASE_DEC, NULL, 0x0,
        NULL, HFILL } },

    { &hf_expiration,
      { "Expiration", "couchbase.expiration",
        FT_UINT32, BASE_DEC, NULL, 0x0,
        NULL, HFILL } },

    { &hf_noreply,
      { "Noreply", "couchbase.noreply",
        FT_STRING, BASE_NONE, NULL, 0x0,
        "Client does not expect a reply", HFILL } },

    { &hf_response,
      { "Response", "couchbase.response",
        FT_STRING, BASE_NONE, NULL, 0x0,
    ...
    { &hf_slabclass,
      { "Slab class", "couchbase.slabclass",
        FT_UINT32, BASE_DEC, NULL, 0x0,
        "Slab class of a stat", HFILL } },

    { &hf_name,
      { "Stat name", "couchbase.name",
        FT_STRING, BASE_NONE, NULL, 0x0,
        "Name of a stat", HFILL } },

    { &hf_name_value,
      { "Stat value", "couchbase.name_value",
        FT_STRING, BASE_NONE, NULL, 0x0,
  ...
  register_dissector ("couchbase.udp", dissect_couchbase_udp, proto_couchbase);
